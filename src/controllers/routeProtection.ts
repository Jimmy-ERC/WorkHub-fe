...
export class RouteProtection {
    ...
    private async checkAuthentication(): Promise<void> {
        try {
            ...
            if (currentPath.endsWith('login.html') || currentPath.endsWith('register.html')) {
                ...
                if (result.user.user_metadata?.user_type === 'Candidato' && !currentPath.includes('/candidate/')) {
                    window.location.href = '/src/pages/candidate/home.html';
                } else if (result.user.user_metadata?.user_type === 'Empresa' && !currentPath.includes('/enterprise/')) {
                    window.location.href = '/src/pages/enterprise/home.html';
                }
            }
    ...
    private redirectToLogin(): void {
        ...
        if (!currentPath.endsWith('register.html') && !currentPath.endsWith('login.html')) {
