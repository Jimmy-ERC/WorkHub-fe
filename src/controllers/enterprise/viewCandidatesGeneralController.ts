...
export class EnterpriseViewCandidatesGeneralController {
  ...
  private async loadCandidates() {
    ...
    const candidates =
      (await JobApplicationsService.getApplicationsByEnterpriseId(
  ...
  public renderCandidates(): void {
    ...
    //=================================
    let filtrosEducacion = Array.from(
      document.querySelectorAll("input[name='educacion']:checked")
    );
    let valoresEducacion: string[] = [];
    if (filtrosEducacion.length > 0) {
      valoresEducacion = filtrosEducacion.map((el) =>
      ...
      if (!valoresEducacion.includes("todos")) {
        ...
        this.filteredCandidates = this.filteredCandidates.filter((c) =>
          valoresEducacion.includes(c.educacion.toLowerCase())
        );
        console.log(
          "candidatos despues de filtrar por educacion: ",
    ...
    if (genero) {
      ...
      console.log(
        "filtrando por genero:" + valorGenero,
        "filtro esperado:otros"
      );
      if (valorGenero !== "todos") {
        console.log("entra a filtrar por genero");
        this.filteredCandidates = this.filteredCandidates.filter(
          (c) => c.genero.toLowerCase() === valorGenero
        );
      }
    }
    console.log(
      "candidatos despues de filtrar por genero: ",
      this.filteredCandidates
    );
    ...
    document.getElementById("nombreVacante")!.innerText =
      (this.filteredCandidates[0]?.nombre_trabajo ||
        "Sin candidatos para la empresa") +
      "- Publicacion: " +
      (this.filteredCandidates[0]?.fecha_publicacion || "Sin fecha");
    ...
    for (const candidate of this.filteredCandidates) {
      ...
      if (this.filteredCandidates.indexOf(candidate) > 0) {
        if (
          candidate.id_trabajo !==
          this.filteredCandidates[
            this.filteredCandidates.indexOf(candidate) - 1
          ]?.id_trabajo
        ) {
          listCandidatos.appendChild(document.createElement("hr"));
          listCandidatos.innerHTML += `<h5 id="nombreVacante">${
            candidate.nombre_trabajo
          } - Publicacion: ${candidate.fecha_publicacion || "Sin fecha"}</h5>`;
        }
      }
      let colorBadge;
      ...
      listCandidatos.innerHTML += `  <div class="card flex row mb-2"
        ...
        candidate.estado || "Pendiente"
                                    ...
                                    <div class="d-flex flex-wrap">
                                        <p class="me-3"><i class="bi bi-geo-alt"></i> ${
                                          candidate.ubicacion
                                        }</p>
                                        <p class="me-3"><i class="bi bi-briefcase"></i> ${
                                          candidate.experiencia
                                        }</p>
                                        <p class="me-3"><i class="bi bi-person"></i> ${
                                          candidate.genero
                                        }</p>
                                        <p class="me-3"><i class="bi bi-book"></i> ${
                                          candidate.educacion
                                        }</p>
                                    </div>
                                </div>
  ...
  public async actualizarEstadoAplicacion(
    idAplicacion: number,
    status: string
  ) {
    const response = await JobApplicationsService.updateApplicationStatus(
      idAplicacion,
      status
    );
    if (response.success) {
  ...
  public llenarModalDetalleCandidato(idCandidato: number) {
    const candidato = this.filteredCandidates.find(
      (c) => c.id_perfil === idCandidato
    );
    if (!candidato) return;
    ...
    if (nacimiento)
      nacimiento.textContent = new Date(
        candidato.fecha_nacimiento_fundacion
      ).toLocaleDateString();
    ...
    const descargaCVCandidato = document.getElementById(
      "descargaCVCandidato"
    ) as HTMLAnchorElement;
    if (descargaCVCandidato) {
      descargaCVCandidato.href = candidato.url_curriculum;
      descargaCVCandidato.download = `${candidato.nombre
        .replace(/\s+/g, "_")
        .toLowerCase()}_cv.pdf`;
    }
    ...
    const sitioWeb = document
      .querySelector("#modalDetalleCandidato .bi-globe")
      ?.parentElement?.querySelector("p.fw-bold");
    if (sitioWeb) sitioWeb.textContent = candidato.pagina_web;

    // Ubicación (en la sección de contacto)
    const ubicacionContacto = document
      .querySelector("#modalDetalleCandidato .bi-geo-alt")
      ?.parentElement?.querySelector("p.fw-bold");
    if (ubicacionContacto) ubicacionContacto.textContent = candidato.ubicacion;

    // Teléfono
    const telefono = document
      .querySelector("#modalDetalleCandidato .bi-telephone")
      ?.parentElement?.querySelector("p.fw-bold");
    if (telefono) telefono.textContent = candidato.telefono;

    // Correo electrónico
    const correo = document
      .querySelector("#modalDetalleCandidato .bi-envelope")
      ?.parentElement?.querySelector("p.fw-bold");
    ...
    if (biografia)
      biografia.textContent = candidato.biografia || "No disponible.";

    const cartaPresentacion = document.getElementById(
      "cartaPresentacionCandidato"
    );
    if (cartaPresentacion) cartaPresentacion.textContent = "No disponible.";

    //boton para enviar correo;
    const enviarCorreoCandidato = document.getElementById(
      "enviarCorreoCandidato"
    ) as HTMLAnchorElement;
    if (enviarCorreoCandidato)
      enviarCorreoCandidato.href = `mailto:${candidato.email}`;

    //icono red social
    const redSocialIconoCandidato = document.getElementById(
      "redSocialIconoCandidato"
    ) as HTMLAnchorElement;
    //limpiamos las clases anteriores
    redSocialIconoCandidato.classList.remove(
      ...redSocialIconoCandidato.classList
    );
    ...
    const redSocialCandidato = document.getElementById(
      "redSocialCandidato"
    ) as HTMLAnchorElement;
...
window.enterpriseViewCandidatesGeneralController =
  new EnterpriseViewCandidatesGeneralController();
