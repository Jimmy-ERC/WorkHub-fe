...
export class EnterpriseHomeController {
    ...
    if (openJobs.length > 0) {
      ...
      openJobs.forEach((job, i) => {
        ...
        if ((i + 1) % 3 === 0 && i !== openJobs.length - 1) {
          openJobsRow!.innerHTML += `<div class="m-2 w-100"></div>`;
        }
      });
    }
  ...
  public llenarModalDetalleTrabajo(idTrabajo: number) {
    ...
    document
      ...
      .addEventListener("click", () =>
        this.verCandidatos(trabajo?.id_trabajo + "")
      );

    // document.getElementById("btnCerrarVacante")!.setAttribute('data-job-id', trabajo!.id_trabajo.toString());

    console.log(
      "Estado del trabajo (true=abierto, false=cerrado):",
      trabajo!.estado
    );

    if (trabajo!.estado) {
      //si el trabajo está abierto, el botón debe permitir cerrarlo
      document.getElementById("btnCambiarEstadoVacante")!.textContent =
        "Cerrar Vacante";
      document.getElementById("btnCambiarEstadoVacante")!.className =
        "btn btn-danger ";

      //cambiamos el texto y clase del botón y del modal según el estado del trabajo
      document.getElementById(
        "modalBodyConfirmacionCerrarVacante"
      )!.textContent = "¿Estás seguro que deseas cerrar esta vacante?";
      document.getElementById("btnConfirmarCambiarEstadoVacante")!.textContent =
        "Sí, cerrar vacante";
      document.getElementById("btnConfirmarCambiarEstadoVacante")!.className =
        "btn btn-danger";

      //asignamos el evento al botón confirmar cerrar vacante
      document
        .getElementById("btnConfirmarCambiarEstadoVacante")
        ?.addEventListener("click", () =>
          this.cambiarEstadoVacante(trabajo!.id_trabajo, trabajo!.estado)
        );
    } else {
      ...
      document.getElementById(
        "modalBodyConfirmacionCerrarVacante"
      )!.textContent = "¿Estás seguro que deseas reabrir esta vacante?";
      document.getElementById("btnConfirmarCambiarEstadoVacante")!.textContent =
        "Sí, reabrir vacante";
      document.getElementById("btnConfirmarCambiarEstadoVacante")!.className =
        "btn btn-success";
  ...
  public verCandidatos(puesto: number | string) {
    ...
    if (isNaN(parseInt(puesto as string))) {
      ...
      puesto =
        this.jobs.find((job) =>
          job.nombre_trabajo.toLowerCase().includes(puesto as string)
        )?.id_trabajo || "-1";
      if (puesto === "-1") {
