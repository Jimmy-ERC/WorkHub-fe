...
export class ProfileEnterpriseService {
  ...
  public static async fetchEnterpriseProfile(): Promise<
    ProfileResponse | ProfileResponseError
  > {
    try {
      if (!user || !user.id) {
        return {
  ...
  public static async createEnterpriseProfile(
    profileData: ProfileResponse["data"]
  ): Promise<ProfileResponse | ProfileResponseError> {
    try {
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Error desconocido al crear el perfil",
      } as ProfileResponseError;
    }
  }

  // funcion para actualizar el perfil empresarial
  public static async updateEnterpriseProfile(
    profileData: ProfileResponse["data"]
  ): Promise<ProfileResponse | ProfileResponseError> {
    try {
      if (!user || !user.id) {
   ...
   */
  public static validateImageFile(file: File): {
    isValid: boolean;
    error?: string;
  } {
    // Check file type
    ...
    if (file.size > maxSize) {
      ...
      return {
        isValid: false,
        error: `El archivo es muy grande (${sizeMB}MB). El tamaño máximo es 5MB.`,
      };
    }
  ...
  public static async verifyBucketConnection(): Promise<boolean> {
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `No se puede acceder al bucket '${bucketName}': ${error.message}`
        );
      }
   ...
   */
  public static async uploadEnterpriseAvatar(
    file: File
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("Bucket not found")) {
          errorMessage =
            "El bucket de almacenamiento no existe. Contacta al administrador.";
        } else if (error.message.includes("No se puede acceder al bucket")) {
          errorMessage = error.message;
        } else if (error.message.includes("row-level security policy")) {
          errorMessage =
            "Las políticas de seguridad no están configuradas. Consulta la documentación de configuración.";
        } else {
          errorMessage = error.message;
        }
      }
