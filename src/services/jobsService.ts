...
export class JobsService {
  ...
  public static async getJobsByProfileId(
    profileId: number
  ...
  public static async closeJob(jobId: number): Promise<JobsResponse> {
    try {
      const response = await fetch(
        `${apiUrl}/enterprise/trabajos/cerrar/${jobId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Error desconocido al cerrar trabajo",
      };
    }
  }
  public static async openJob(jobId: number): Promise<JobsResponse> {
    try {
      const response = await fetch(
        `${apiUrl}/enterprise/trabajos/abrir/${jobId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Error desconocido al abrir trabajo",
      };
    }
  }
