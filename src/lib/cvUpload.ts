...
function createCVItemHTML(cv: Curriculum): string {
    const fileExtension = CurriculumService.getFileExtension(cv.url_curriculum).toUpperCase();
...
function attachCVItemListeners(cv: Curriculum) {
    // Botón editar nombre
    const editBtn = document.querySelector(`[data-action="edit-cv-name"][data-cv-id="${cv.id_curriculum}"]`);
    if (editBtn) {
        editBtn.addEventListener('click', () => handleEditCVName(cv.id_curriculum));
    }

    // Botón eliminar
    const deleteBtn = document.querySelector(`[data-action="delete-cv"][data-cv-id="${cv.id_curriculum}"]`);
    if (deleteBtn) {
        deleteBtn.addEventListener('click', () => handleDeleteCV(cv.id_curriculum, cv.url_curriculum));
    }

    // Click en el nombre para descargar
    const nameElement = document.querySelector(`.cv-item-name[data-cv-id="${cv.id_curriculum}"]`);
    if (nameElement) {
...
async function handleEditCVName(cvId: number) {
    const nameElement = document.querySelector(`.cv-item-name[data-cv-id="${cvId}"]`) as HTMLElement;
    const editBtn = document.querySelector(`[data-action="edit-cv-name"][data-cv-id="${cvId}"]`) as HTMLButtonElement;
    ...
    if (isEditing) {
        ...
        try {
            const result = await CurriculumService.updateCurriculumName(cvId, newName);
...
async function handleDeleteCV(cvId: number, cvUrl: string) {
    const confirmed = confirm('¿Estás seguro de que deseas eliminar este CV? Esta acción no se puede deshacer.');
    ...
    const deleteBtn = cvItem.querySelector(`[data-action="delete-cv"]`) as HTMLButtonElement;
    if (deleteBtn) {
...
export async function reloadCVList(idPerfil?: number) {
    try {
        ...
        if (!profileId) {
            ...
            const profileResponse = await fetch(`${import.meta.env.VITE_API_URL}/candidate/perfiles/${user.id}`);
...
async function handleCVUpload(file: File, idPerfil: number) {
    try {
        ...
        showToast(error instanceof Error ? error.message : 'Error desconocido al subir el CV', 'danger');
    } finally {
        resetCVDropzone();
    }
}
