...
export class FormValidator {
    public static validateField(field: FormField): string[] {
        ...
        for (const rule of field.rules) {
            ...
            if (rule.minLength && field.value.length < rule.minLength) {
                errors.push(rule.message || `${field.name} must be at least ${rule.minLength} characters`);
            }

            if (rule.maxLength && field.value.length > rule.maxLength) {
                errors.push(rule.message || `${field.name} must be no more than ${rule.maxLength} characters`);
            }
    ...
    public static validateForm(fields: FormField[]): { isValid: boolean; errors: Record<string, string[]> } {
